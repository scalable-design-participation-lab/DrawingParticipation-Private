...
describe('SidebarComponent', () => {
  ...
  it('activates drawing modes frequency correctly', async () => {
    ...
    expect(mockDrawingStore.activateDrawing).toHaveBeenCalled('every week')


    await wrapper.vm.spaceContent.buttonGroup[2].action()
    expect(mockDrawingStore.activateDrawing).toHaveBeenCalled('sometimes')

    await wrapper.vm.spaceContent.buttonGroup[3].action()
    expect(mockDrawingStore.activateDrawing).toHaveBeenCalled('only once')

    await wrapper.vm.spaceContent.buttonGroup[4].action()
    expect(mockDrawingStore.activateDrawing).toHaveBeenCalled('never')
  })
